...
ActiveRecord::Base.connection.execute("DELETE from sqlite_sequence where name = 'stores_users'")
...
r1 = Role.create({name: "vendedor", description: "Puede ver centros, rubros, locales y ventas. Puede actualizar/eliminar sus ventas de su local"})
r2 = Role.create({name: "supervisor", description: "Puede ver centros, rubros, locales y ventas. Puede actualizar/eliminar todas las ventas de su local"})
r3 = Role.create({name: "administrador", description: "Puede crear/ver/modificar/borrar cualquier registro"})
...
u3 = User.create({name: "Ignacio", email: "itorres@srichile.cl", password: "aaaaaaaa", password_confirmation: "aaaaaaaa", role_id: r3.id})

stu1 = StoresUser.create({store_id: st1.id, user_id: u1.id})
stu1 = StoresUser.create({store_id: st2.id, user_id: u2.id})
stu1 = StoresUser.create({store_id: st3.id, user_id: u3.id})
